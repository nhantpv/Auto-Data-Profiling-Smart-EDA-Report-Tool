...
**Cách hoạt động:** Cả 3 thuật toán cùng quét qua dữ liệu và chấm cho từng dòng một số điểm gọi là **Anomaly Score** (Điểm dị biệt). Để tối ưu và tránh pha loãng tín hiệu, hệ thống sử dụng cơ chế **Max Z-score combiner** (quy chuẩn điểm Z-score của từng thuật toán rồi lấy giá trị lớn nhất). Hệ thống sẽ thiết lập một ngưỡng (tự động tra cứu `calibrator_table.json`) để ném các dòng này sang cho Layer 3 đóng gói vào JSON.

**Lưu ý quan trọng (Xử lý ô trống):** Thuật toán PyOD rất "dị ứng" với các ô trống (NaN) và sẽ báo lỗi nếu gặp phải. Hệ thống giải quyết bằng cách tự động lọc bỏ các dòng chứa giá trị trống (dropna) trên các cột số trước khi huấn luyện và dự đoán, đảm bảo PyOD chạy trơn tru và phát hiện chính xác các outlier đa biến thực sự mà không bị nhiễu bởi dữ liệu khuyết.
